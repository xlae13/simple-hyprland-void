...
function check_os {
    if [ -f /etc/os-release ]; then
        . /etc/os-release
        if [[ "$ID" != "void" ]]; then
            print_warning "This script is designed for Void Linux. Your system: $PRETTY_NAME"
            if ! ask_confirmation "Continue anyway?"; then
                log_message "Installation cancelled due to unsupported OS"
                exit 1
            fi
        else
            print_success "Void Linux detected. Proceeding with installation."
            log_message "Void Linux detected. Installation proceeding."
        fi
    else
